...
async function fetchPage() {
    ...
    try {
        ...
        const everything = await Promise.all($('.news_area').map(async (i, el) => {

            const headline = $(el).find('.news_contents .news_tit').prop('title');
            let press = $(el).find('.news_info .info_group .info.press').not('i').text();
            const newsContent = $(el).find('.news_dsc .dsc_wrap a').text();
            const newsImage= $(el).find('.news_contents .dsc_thumb img').prop('data-lazysrc');
